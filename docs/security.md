...
Linear GraphQL calls use `linear.apiTimeoutMs`, defaulting to 8000 ms, so stalled API requests are recorded as local errors instead of indefinitely blocking acknowledgement or routing work.
Inbox Notification webhooks are accepted without queueing Codex work and are written to the local audit trail. Direct mention/comment notifications are copied into matching active job timelines, but they do not start new Codex work by themselves. If Linear reports `issueUnassignedFromYou`, or an `issueStatusChanged` notification with a completed or canceled status type, Tetherbox cancels active local jobs matching that Linear issue because the app should stop local work that is no longer active in Linear.
