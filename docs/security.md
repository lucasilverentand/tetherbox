...
Linear GraphQL calls use `linear.apiTimeoutMs`, defaulting to 8000 ms, so stalled API requests are recorded as local errors instead of indefinitely blocking acknowledgement or routing work. When Linear includes `organizationId`, Tetherbox uses the matching stored app actor installation for Agent Activity, Agent Session updates, repository suggestions, and issue lifecycle updates, with the legacy default installation as a fallback.
Inbox Notification webhooks are accepted without queueing Codex work and are written to the local audit trail. Mention, comment, assignment, and reaction notifications are copied into matching active job timelines, but they do not start new Codex work by themselves. If Linear reports `issueUnassignedFromYou`, or an `issueStatusChanged` notification with a completed or canceled status type, Tetherbox cancels active local jobs matching that Linear issue because the app should stop local work that is no longer active in Linear.
Permission-change webhooks are accepted without queueing Codex work and are written to the local audit trail. OAuth app revocation webhooks remove the matching organization-scoped Linear installation token when Linear includes organization context, falling back to the legacy default installation for older payloads. A warning is recorded so the daemon stops using stale app credentials until the app is installed again.
