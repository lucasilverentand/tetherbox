...
When `server.publicUrl` is configured, the first Agent Session update adds the local Tetherbox dashboard job link with `addedExternalUrls`, preserving any existing Linear session links.
After successful implementation, created or updated GitHub pull requests are added to the Linear Agent Session with `addedExternalUrls`. When `server.publicUrl` is configured, the session also keeps a link back to the selected local Tetherbox dashboard job without replacing existing session links. Tetherbox then moves the source issue to `linear.reviewStateName`, defaulting to `In Review`, when that state exists for the issue's team.
