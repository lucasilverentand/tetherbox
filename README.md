...
When `server.publicUrl` is configured, the first Agent Session acknowledgement sets Linear's current `externalUrls` to the local Tetherbox job URL so users can open the active local job immediately.
